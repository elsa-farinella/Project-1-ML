...
def main():
    ...
    # Then it's time to split the data into training and a local validation set
    train_data, val_data = split_data(data, y, 0.95, seed=13)

    # We split the data into features and labels, since they were put together inside the previous function in ordrer not to lose the correspondence between them
    val_x = val_data[:, 0:-1]
    val_y = val_data[:, -1]

    # Before doing the same ting for train data, we drop some rows on it, so that we can train our model on a balanced dataset
    ...
    # Now we can splil features and labels also in train set
