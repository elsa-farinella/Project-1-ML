...
def logistic_regression(y, tx, initial_w, max_iters, gamma):
    ...
    for n_iter in range(max_iters):
        ...
        print(
            "SGD Logistic Regression iter. {bi}/{ti}: loss={l}, w={w}".format(
                bi=n_iter, ti=max_iters - 1, l=loss, w=w
            )
        )
    return w, loss
...
def reg_logistic_regression(y, tx, lambda_, initial_w, max_iters, gamma, print = True):
    w = initial_w
    for n_iter in range(max_iters):
        loss = compute_logistic_loss(y, tx, w) + lambda_ * np.linalg.norm(w)**2
        # index = np.random.randint(0, len(y), 1)
        # y_batch = y[index]
        # tx_batch = tx[index]
